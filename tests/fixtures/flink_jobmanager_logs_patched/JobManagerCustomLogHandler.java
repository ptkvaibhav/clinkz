 ...
 *
 * <p>This is the FIXED (post-CVE-2020-17519) variant, commit
 * b561010b0ee741543c3953306037f00d7a9f0801 — a negative control fixture. The
 * only change vs the vulnerable variant is that the path parameter is wrapped in
 * {@code new File(...).getName()} to strip any directory information before it
 * reaches the {@code new File(logDir, filename)} sink. The discovery engine must
 * read this basename-strip guard as SANCTIONED (Δ removed) and emit NO hypothesis
 * — N/A by construction on a patched instance.
 */
