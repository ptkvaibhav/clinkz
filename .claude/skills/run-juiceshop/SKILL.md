...
6. If coverage < applicable count, give a single-line root cause per missed category and bucket it as one of: **(a) JWT-auth failure** (JSON/API auth is now wired — bucket here only if the JWT session genuinely failed to establish), **(b) SPA/API endpoint discovery** (crawler saw only the SPA shell, never the `/api`+`/rest` routes), **(c) JSON-body param handling** (params live in a JSON request body, not the query string our extraction reads), or **(d) genuine methodology gap**. This bucketing is the deliverable — it prioritises the next fix. Note: as of the JWT-auth + `is_http` fixes, buckets (b) SPA discovery and (c) JSON-body params are the expected dominant blockers.
