...
1. Verify Juice Shop is running at http://localhost:3000 (check via `Invoke-WebRequest http://localhost:3000` or `docker compose -f docker/docker-compose.yml ps`). If not running: `docker compose -f docker/docker-compose.yml up -d juiceshop`
   ...
   - Juice Shop self-registration is open: `POST /api/Users` `{email, password, passwordRepeat}` then `POST /rest/user/login` `{email, password}` returns the JWT at `data.authentication.token`. This is how `tests/test_skills_juiceshop/conftest.py` authenticates — the pipeline's cookie-based `WebAuthenticator` does NOT do this.
...
6. If coverage < applicable count, give a single-line root cause per missed category and bucket it as one of: **(a) JWT-auth not wired** (cookie flow can't establish a Juice Shop session), **(b) SPA/API endpoint discovery** (crawler saw only the SPA shell, never the `/api`+`/rest` routes), **(c) JSON-body param handling** (params live in a JSON request body, not the query string our extraction reads), or **(d) genuine methodology gap**. This bucketing is the deliverable — it prioritises the next fix.
...
- **Auth**: Did a JWT session establish, or did the cookie-based `WebAuthenticator` GET→form→POST flow fail to log in? (Single most likely break — Juice Shop has no HTML login form to parse and returns a JWT in a JSON body, neither of which the cookie flow handles.)
- **Discovery**: Did scan/crawl surface `/api/*` and `/rest/*` routes, or only the SPA `index.html` shell? Angular routes aren't anchor links — katana/gospider/hakrawler crawling HTML won't see them.
