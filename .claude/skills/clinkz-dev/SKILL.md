...
**The seam-versus-pipeline law (same family, fourth face).** A domain excludes the member it hunts. A pattern excludes the shape it hunts. An acceptance criterion measures something adjacent to what it certifies. A **seam test excludes the producer.**

> The login-discovery fix was verified by calling the authenticator with the login URL handed in. It passed at every spelling — which is what the fix was about — and it could not have failed for the reason the pipeline did. The authentication phase runs BEFORE scan, so `_find_login_url` ran with `scan_result=None` and the state store's endpoint table empty, and the shape ranker whose name filter had just been removed had **nothing to rank**. The seam was correct and the run still POSTed the credentials at the site root.

So: **a component tested with its inputs supplied cannot reveal a defect in how those inputs are produced.** Handing in the input is what makes a seam test sharp — it isolates the component — and it is exactly what removes the producer from the experiment. **Any fix verified at a seam must also be verified through the pipeline that feeds it**, on a target whose shape forces the pipeline to do the producing.

The same three questions, asked of a seam test:

1. **What does this test supply that the pipeline must discover?** Every argument is a candidate. Write down where each one comes from in a real run, and whether that producer has run yet at the point of the call.
2. **Could the pipeline hand this component something the test never does?** `None`, `[]`, a partial result from a phase still running, a value from a phase that has not started. An argument the test always fills is an argument the pipeline may not.
3. **Would this test go red if the producer stopped producing?** If not, it is a test of the consumer only, and it must be paired with one that is not.

The pipeline check is not a second unit test — it is the run: `python -m clinkz scan --target <url>`, read out of the report and the trace. And **the phase order is part of the API**: a consumer reading a producer that runs after it gets that producer's *default*, not its output, and a default is byte-identical to a real answer of the same value. Before wiring a phase to another phase's result, state which one runs first.

**The type-conditioned-rule law (same family, fifth face).** A domain excludes the member it hunts. A pattern excludes the shape it hunts. An acceptance criterion measures something adjacent. A seam test excludes the producer. A **rule conditioned on a value's TYPE has a domain that moves when a producer's type moves** — and it moves silently, in the diff that moved the type.

> `redact_structure`'s key-aware branch read `isinstance(key, str) and isinstance(value, str)`. Every credential key whose value was a `str` was redacted; any whose value was a `list` stepped past the branch into the generic walk, where each element meets only the SHAPE rules — and a session cookie the target named has no shape. The same push that added `HTTPClientOutput.set_cookie: list[str]` — carrying `Set-Cookie` verbatim, `model_dump()`-ed into `parsed_output`, straight through the trace writer into `outputs/<id>/invocations/` — was the push that moved a credential-bearing field's type from `str` to `list[str]`. **The diff and its guard shipped together and the guard did not cover the diff.** The header DICT beside it was redacted correctly the whole time, which is what made the gap easy to miss.

The guard-domain law asks *"if somebody adds a new member, does this go red?"*. This is the same question one level down: **"if somebody changes a member's TYPE, does this go red?"** A guard whose table is a set of NAMES answers yes to the first and no to the second, because the name did not change — the shape of what is behind it did.

Two rules follow.

1. **A rule that decides by type must enumerate the types it handles, and fail closed on the rest.** Not "if `str`, redact; else walk" — `else` is where the next type lands. A container the walker claims to handle is a container the guard exercises, and a container it does not handle is a refusal, not a fall-through.
2. **Derive the key set from the MODELS, not from a name list crossed with a container set.** `CREDENTIAL_HEADER_KEYS` is a list of spellings; a field renamed `set_cookie` → `set_cookies` (both spellings are live in this codebase today, on two different models) leaves the list intact and the field unprotected. **A name list does not survive a rename; an annotation does**, because the annotation moves with the field.

**Priced, not built.** The annotation form is `Annotated[list[str], Credential(kind="cookie")]` on the producing field, plus a walk over every `BaseModel` subclass under `src/clinkz/` collecting annotated fields into the redactor's key set at import. Cost: ~40 lines in `engagement/credential_shapes.py`, a `Credential` marker type, one bidirectional guard asserting the derived set against `CREDENTIAL_HEADER_KEYS` (which stays, for the HEADER names that belong to no model), and an audit of every credential-bearing model field to place the annotations — 9 fields across `tools/http_client.py`, `tools/auth.py`, `models/engagement.py` and `tools/redirect_walk.py` on the current tree. The risk is the audit, not the mechanism: a field the audit misses is unprotected and looks protected, which is the same failure with a new source. So it lands with the name list kept as a floor and the derived set asserted to be a SUPERSET of it, never a replacement, until the audit has a witness for every field.
