...
- **Methodology correctness — real-attacker confirmation (Juice Shop 8df94e28)**: three platform-agnostic fixes so a methodology confirms the way a real attacker does.
  ...
  - **Open-redirect allowlist bypass**: a redirect-shaped param (`to`/`redirect`/`url`/`next`/…) whose generic probes are all blocked (Juice Shop `/redirect?to=` → 406) no longer reads as a non-finding. `_harvest_allowlist_tokens` fetches the app's origin root + same-origin JS bundles and extracts the absolute URLs it wraps in its own redirect param (the allowlist members it links to), then `_allowlist_bypass_payloads` embeds each token in an attacker URL (`https://evil.example/?x=<token>`, path, fragment, `@`-userinfo). Confirmation is a 3xx `Location` (or body redirect) pointing at the attacker host despite the substring allowlist (`RedirectBypassType.ALLOWLIST_BYPASS`).
