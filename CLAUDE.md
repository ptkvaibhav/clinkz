...
- **Methodology correctness — real-attacker confirmation (Juice Shop 8df94e28)**: three platform-agnostic fixes so a methodology confirms the way a real attacker does.
  ...
  - **Open-redirect — honest off-site confirmation + DVWA-graded + allowlist bypass**: confirmation is **browser-faithful host resolution** (`_redirect_target_is_attacker` resolves the `Location`/body-redirect against the page URL via `urljoin` and confirms the effective navigation host is the attacker host) rather than a loose `"evil.example" in target` substring + verbatim-echo fallback — so a same-host path that merely *contains* the attacker string (DVWA Low echoing `info.phphttps://evil.example`, or `/go?u=https://evil.example`) is a justified non-finding, not a phantom ("a redirect to an in-site page is not the finding"). A `direct` absolute-URL probe classifies the no-validator (DVWA Low) case as `validator_type="none"`. The **allowlist bypass is a fallback whenever a redirect-shaped param (`to`/`redirect`/`url`/`next`/…) is not confirmed by the standard probes** — covering both Juice Shop's `/redirect?to=` (every generic probe 406s, phase-1 sees no divergence) and DVWA **High** (which requires the target to *contain* `info.php`, so the appended attacker URL is *accepted but echoed same-host*, making phase 1 a candidate that the standard path then can't confirm). `_harvest_allowlist_tokens` fetches the **page-under-test's own base path** + origin root + same-origin bundles and harvests both **absolute outbound-allowlist URLs** (Juice Shop) and **relative path/file tokens** the app assigns to a redirect param (DVWA High's `?redirect=info.php` → `info.php`); `_allowlist_bypass_payloads` embeds each token in an attacker URL (`https://evil.example/?x=<token>`, path, fragment, `@`-userinfo), confirmed by host-resolution despite the substring allowlist (`RedirectBypassType.ALLOWLIST_BYPASS`). DVWA open-redirect now confirms at **Low** (direct off-site), **Medium** (protocol-relative; absolute `http(s)://` blocked), and **High** (substring-allowlist bypass); **Impossible** is a justified non-finding (ID-based, no attacker-controlled destination). Severity medium (high for `js:`/`data:`).
