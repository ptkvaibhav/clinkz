...
def build_dry_run_plan(
    ...
    plan.refused_categories = [f"{label} [{category}]" for category, label in _REFUSED_CATEGORIES]
...
def _demonstrate_refusals(scope: EngagementScope) -> list[str]:
    ...
    for method, path in _DEMO_REQUESTS:
        ...
        status = f"REFUSE [{verdict.category}]" if verdict.refused else "allow"
...
def render_dry_run(plan: DryRunPlan) -> str:
    ...
    lines += [f"  - {entry}" for entry in plan.refused_categories]
...
__all__ = ["DryRunClass", "DryRunPlan", "build_dry_run_plan", "render_dry_run"]
