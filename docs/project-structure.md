...
│                     #   _prototype_pollution (the CWE-1321 oracle — its input
│                     #   type carries a status and headers and NO BODY, because
│                     #   the polluting request's own response answers in the
...
docker/  scripts/  tests/  docs/
                      ...
                      #     the oracle rests on. Three merge endpoints, one
                      #     vulnerability; /internal/_reset is bound to loopback
                      #     plus a header no clinkz code path sends, so the engine
                      #     cannot un-pollute itself mid-run
                      #   docker-compose.yml::umami — the adaptive-auth HONESTY
                      #     CONTROL. A real Next.js App Router app that does NOT
                      #     use NextAuth: same X-Powered-By and RSC Vary as
                      #     cal.diy, no csrfToken field, credential sink at
                      #     POST /api/auth/login returning a bearer token
